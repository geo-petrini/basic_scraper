...
def parse_page(page):
    ...
    cards = soup.find_all('div', class_="s-card-container") #cards = lista di elementi corrispondenti alla ricerca fatta da "find_all"
...
def get_product_reviews_count(card):
    ...
    # print(d)
    if d:
        rc = int((d.select('span:nth-of-type(2)')[0]['aria-label']).split(' ')[0].replace('.',''))
    return rc    
